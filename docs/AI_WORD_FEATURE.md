...
>   - `canonical_text`: the word's canonical form in the target language (lowercase, stripped of articles/punctuation, with diacritics restored). Verbs must use the infinitive.
>   - `part_of_speech`: one controlled grammatical value, including `verb`, `noun`, or `adjective`.
>   - `cefr_level`: exactly one of `A1`, `A2`, `B1`, `B2`, `C1`, or `C2`.
...
Context and questions are sent as separate quoted fields in the **user message**, never in the system prompt. They also establish the privacy boundary: only a lookup with neither context nor a question can populate the shared AI cache and trainer inventories. Contextual answers remain in `user_word_lookups.result_data`.
...
- **`words`** itself is the index — `(text, language_id)` unique. A context-free AI lookup stores the validated CEFR estimate in `Word.cefr_level` when no curated level already exists.

- **`word_translations`** receives provisional, unverified copies of context-free translations so newly discovered words are immediately eligible for the Words trainer. Existing verified rows are never replaced.

- **Verb promotion** — a `part_of_speech="verb"` result gets the generic `verb` tag. Its canonical infinitive, CEFR level, translations, and applicable tags are synchronized into `verbs`, `verb_translations`, and `verb_tags`. These translations remain unverified and visible in the admin review inventory. Conjugations are not generated by Add Word.

- **Difficulty tags** — the scalar `Word.cefr_level` / `Verb.cefr_level` is canonical. The matching `a1`–`c2` tag is synchronized for smart-set compatibility, and conflicting difficulty tags are removed.
...
- Creates: `Word(EN, "ephemeral", cefr_level=...)`, `WordLexicalEntry`, `WordNativeTranslation(FR)`, and an unverified `WordTranslation(FR)` trainer row.
